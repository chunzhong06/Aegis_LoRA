...
def create_new_session(name, base_path, lora_path, cleanse_mode, sessions):
    ...
    # 正式审计入口只接受标准 safetensors LoRA。
    required_lora_files = ("adapter_config.json", "adapter_model.safetensors")
    missing_lora_files = [
        filename for filename in required_lora_files if not (lora_path / filename).is_file()
    ]
    if missing_lora_files:
        missing_text = ", ".join(missing_lora_files)
        yield (
            gr.update(),
            sessions,
            f"🔴 LoRA 路径无效，缺少标准文件: {missing_text}",
